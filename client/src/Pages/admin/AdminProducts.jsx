    ...
    <div className="space-y-4 md:space-y-6 pb-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="w-full sm:w-auto">
          ...
          <p className="text-gray-600 mt-1 text-sm">Total: <span className="font-bold text-yellow-500">
            {products.length}</span></p>
        </div>
        <button
          onClick={() => {
      ...
      {selectedProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-screen overflow-y-auto">
            ...
            p-4 sm:p-6 flex justify-between items-start sticky top-0">
              <h3 className="text-lg sm:text-2xl font-bold pr-4">{selectedProduct.name}</h3>
              <button onClick={() => setSelectedProduct(null)} 
              className="p-2 hover:bg-yellow-600 rounded-lg flex-shrink-0">
                <X size={20} />
              </button>
            </div>
